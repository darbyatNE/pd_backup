...
export function LoadForecastChart({ profile, xAxis, onXAxisChange }: LoadForecastChartProps) {
    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {view === '3d' && (
            <span className="text-xs text-slate-400 italic">X = hours · Z = months (date) · Y = MW</span>
          )}
        </div>
      </div>
        ...
        <div className="flex items-center gap-5 mb-3 text-xs text-slate-500 flex-wrap">
          <div className="flex items-center gap-1.5">
            ...
            <span className="font-medium">
              Capacity {getSiteCapacityForYear(profile, activeYear)} MW
              {profile.capacityByYear && Object.keys(profile.capacityByYear).length > 1 && !showFullScope && (
